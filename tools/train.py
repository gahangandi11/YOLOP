...
def main():
    ...
    if rank in [-1, 0]:
        ...
        if os.path.exists(cfg.MODEL.PRETRAINED_DET):
            logger.info("=> loading model weight in det branch from '{}'".format(cfg.MODEL.PRETRAINED_DET)) ###chnaged from (cfg.MODEL.PRETRAINED to  adding _det at the end
            det_idx_range = [str(i) for i in range(0,23)] 
