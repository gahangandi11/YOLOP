...
def main():
    ...
    checkpoint_file = args.weights[0] ############# added 0 otherwise I was getting an error
    logger.info("=> loading checkpoint '{}'".format(checkpoint_file))
    checkpoint = torch.load(checkpoint_file)
    # checkpoint = torch.load(checkpoint_file,map_location=torch.device('cpu') ) ############################## I had to add this for CPU's sake
