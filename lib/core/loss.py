...
class MultiHeadLoss(nn.Module):
    ...
    def __init__(self, losses, cfg, model, lambdas=None): ############
        ...
        self.hyp_box=7.5*0.02  # box loss gain ##changing this from 0.05 to 0.02
        self.hyp_cls =0.5*0.02  # cls loss gain (scale with pixels)
        self.hyp_dfl =1.5*0.02  # dfl loss gain
    ...
    def _forward_impl(self, predictions, targets, shapes, batch_size):
        ...
        loss = lbox + ldfl + lcls  + lseg_ll + liou_ll

        # print('lseg_da, lseg_ll, liou_ll:', lseg_da, lseg_ll,liou_ll)
        # print('============================')
        # loss = lseg
        # return loss * bs, torch.cat((lbox, lobj, lcls, loss)).detach()
        return loss, (lbox.item(), ldfl.item(), lcls.item(), lseg_ll.item(), liou_ll.item(), loss.item())
