...
from lib.core.general import check_img_size,scale_coords,xyxy2xywh,xywh2xyxy,box_iou,coco80_to_coco91_class,plot_images,ap_per_class,output_to_target
...
from .ops import non_max_suppression as nms
...
def train(cfg, train_loader, model, criterion, optimizer, scaler, epoch, num_batch, num_warmup,
    ...
    for i, (input, target, paths, shapes) in enumerate(train_loader):
        ...
        with amp.autocast(enabled=device.type != 'cpu'):
            outputs = model(input)
            total_loss, head_losses = criterion(outputs, target, shapes, batch_size = (cfg.TRAIN.BATCH_SIZE_PER_GPU)*2)
            # print(head_losses)
...
def validate(epoch,config, val_loader, val_dataset, model, criterion, output_dir,
    ...
    for batch_i, (img, target, paths, shapes) in tqdm(enumerate(val_loader), total=len(val_loader)):
        ...
        with torch.no_grad():
            ...
            total_loss, head_losses = criterion((train_out, ll_seg_out), target, shapes, batch_size = (config.TEST.BATCH_SIZE_PER_GPU)*2)   #Compute loss ############
            ...
            output = nms(inf_out, conf_thres= config.TEST.NMS_CONF_THRESHOLD, iou_thres=config.TEST.NMS_IOU_THRESHOLD, labels=lb)
